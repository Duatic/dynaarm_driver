// Code taken from:
// https://github.com/ros-controls/gz_ros2_control/blob/jazzy/gz_ros2_control/src/gz_ros2_control_plugin.cpp Needs to be
// ported since GazeboSimSystem is linked as pointer and Private.
...
#include "dynaarm_gz_ros2_control/gz_ros2_control_plugin.hpp"
...
{
public:
  GZResourceManager(rclcpp::Node::SharedPtr& node, sim::EntityComponentManager& ecm,
    ...
    , dynaarm_system_loader_("dynaarm_gz_ros2_control", "gz_ros2_control::GazeboSimSystemInterface")
    , logger_(node->get_logger().get_child("GZResourceManager"))
  {
    RCLCPP_ERROR(logger_, "GZResourceManager initialized with node: %s", node->get_name());

    // Store the node, ecm, and enabledJoints for later use
    node_ = node;
    ecm_ = &ecm;
  ...
  bool load_and_initialize_components(const std::string& urdf, unsigned int update_rate) override
  {
    ...
    RCLCPP_WARN(logger_, "Parsed hardware info from URDF: %zu joints", hardware_info.size());

    for (const auto& individual_hardware_info : hardware_info) {
      std::string robot_hw_sim_type_str_ = individual_hardware_info.hardware_plugin_name;
      RCLCPP_WARN(logger_, "Load hardware interface %s ...", robot_hw_sim_type_str_.c_str());
      ...
      try {
        gzSimSystem = std::unique_ptr<gz_ros2_control::GazeboSimSystemInterface>(
            dynaarm_system_loader_.createUnmanagedInstance(robot_hw_sim_type_str_));
      } catch (pluginlib::PluginlibException& ex) {
        RCLCPP_ERROR(logger_, "The plugin failed to load for some reason. Error: %s\n", ex.what());
        continue;
      }
      RCLCPP_WARN(logger_, "Loaded hardware interface %s", robot_hw_sim_type_str_.c_str());

      // initialize simulation requirements
      if (!gzSimSystem->initSim(node_, enabledJoints_, individual_hardware_info, *ecm_, update_rate)) {
        RCLCPP_FATAL(logger_, "Could not initialize robot simulation interface");
        components_are_loaded_and_initialized_ = false;
        break;
      }
      RCLCPP_WARN(logger_, "Initialized robot simulation interface %s!", robot_hw_sim_type_str_.c_str());
...
private:
  ...
  pluginlib::ClassLoader<gz_ros2_control::GazeboSimSystemInterface> dynaarm_system_loader_;
...
GazeboSimROS2ControlPluginPrivate::GetEnabledJoints(const sim::Entity& _entity, sim::EntityComponentManager& _ecm) const
{
  ...
  for (const auto& jointEntity : jointEntities) {
    const auto jointName = _ecm.Component<sim::components::Name>(jointEntity)->Data();
    ...
    switch (jointType->Data()) {
      ...
      {
        RCLCPP_INFO(node_->get_logger(), "[dynaarm_gz_ros2_control] Fixed joint [%s] (Entity=%lu)] is skipped",
      ...
      case sdf::JointType::UNIVERSAL:
      {
        RCLCPP_WARN(node_->get_logger(),
                    "[dynaarm_gz_ros2_control] Joint [%s] (Entity=%lu)] is of unsupported type."
      ...
      default:
      {
        RCLCPP_WARN(node_->get_logger(), "[dynaarm_gz_ros2_control] Joint [%s] (Entity=%lu)] is of unknown type",
                    jointName.c_str(), jointEntity);
        continue;
      }
    }
...
//////////////////////////////////////////////////
void GazeboSimROS2ControlPlugin::Configure(const sim::Entity& _entity, const std::shared_ptr<const sdf::Element>& _sdf,
                                           sim::EntityComponentManager& _ecm, sim::EventManager&)
{
  ...
  if (paramFileName.empty()) {
    RCLCPP_ERROR(logger, "Gazebo ros2 control found an empty parameters file. Failed to initialize.");
    return;
  }
  ...
  if (sdfPtr->HasElement("pid_update_rate")) {
    int pid_update_rate = sdfPtr->GetElement("pid_update_rate")->Get<int>();
    this->dataPtr->update_rate = pid_update_rate;
  } else {
    RCLCPP_ERROR(logger, "Gazebo ros2 control plugin requires a pid_update_rate parameter in the SDF. "
                         "Failed to initialize.");
    return;
  }
  RCLCPP_INFO(logger, "Using pid update rate: %u", this->dataPtr->update_rate);
  ...
  RCLCPP_DEBUG_STREAM(this->dataPtr->node_->get_logger(), "[Gazebo Sim ROS 2 Control] Setting up controller for ["
  ...
  if (enabledJoints.size() == 0) {
    RCLCPP_DEBUG_STREAM(this->dataPtr->node_->get_logger(), "[Gazebo ROS 2 Control] There are no available Joints.");
    return;
  }

  std::unique_ptr<hardware_interface::ResourceManager> resource_manager_ =
      std::make_unique<dynaarm_gz_ros2_control::GZResourceManager>(this->dataPtr->node_, _ecm, enabledJoints);
  ...
  this->dataPtr->control_period_ = rclcpp::Duration(std::chrono::duration_cast<std::chrono::nanoseconds>(
...
//////////////////////////////////////////////////
void GazeboSimROS2ControlPlugin::PreUpdate(const sim::UpdateInfo& _info, sim::EntityComponentManager& /*_ecm*/)
{
  if (!this->dataPtr->controller_manager_) {
    return;
  }
  ...
  if (!warned) {
    ...
    if (this->dataPtr->control_period_ < _info.dt) {
      RCLCPP_ERROR_STREAM(this->dataPtr->node_->get_logger(), "Desired controller update period ("
                                                                  ...
                                                                  << " s) is faster than the gazebo simulation "
                                                                     "period ("
...
//////////////////////////////////////////////////
void GazeboSimROS2ControlPlugin::PostUpdate(const sim::UpdateInfo& _info, const sim::EntityComponentManager& /*_ecm*/)
{
  if (!this->dataPtr->controller_manager_) {
    return;
  }
  ...
  if (sim_period >= this->dataPtr->control_period_) {
    ...
    auto gz_controller_manager =
        std::dynamic_pointer_cast<gz_ros2_control::GazeboSimSystemInterface>(this->dataPtr->controller_manager_);
...
GZ_ADD_PLUGIN(dynaarm_gz_ros2_control::GazeboSimROS2ControlPlugin, gz::sim::System,
              dynaarm_gz_ros2_control::GazeboSimROS2ControlPlugin::ISystemConfigure,
              dynaarm_gz_ros2_control::GazeboSimROS2ControlPlugin::ISystemPreUpdate,
              dynaarm_gz_ros2_control::GazeboSimROS2ControlPlugin::ISystemPostUpdate)
GZ_ADD_PLUGIN_ALIAS(dynaarm_gz_ros2_control::GazeboSimROS2ControlPlugin, "ign_ros2_control::"
