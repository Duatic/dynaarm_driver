...
{
    hardware_interface::CallbackReturn DynaArmHardwareInterface::on_init_derived(const hardware_interface::HardwareInfo &system_info)
    {
        // configure ethercat bus and drives
        const auto ethercat_bus = system_info.hardware_parameters.at("ethercat_bus");
        const ecat_master::EthercatMasterConfiguration ecat_master_config = {.name = "DynaArmHardwareInterface", .networkInterface = ethercat_bus, .timeStep = 0.001}; // TODO set timestep according to the update rate of ros2control (or spin asynchronously)
        ...
        for (int i = 0; i < static_cast<int>(info_.joints.size()); i++)
        {
            ...
            const std::string device_file_path = ament_index_cpp::get_package_share_directory("dynaarm_hardware_interface") + "config/dynadrive.yaml";
        ...
        RCLCPP_INFO_STREAM(logger_, "Successfully started Ethercat Master on Network Interface: "
    ...
    hardware_interface::CallbackReturn DynaArmHardwareInterface::on_activate_derived(const rclcpp_lifecycle::State & /*previous_state*/)
    {
        ...
        for (int i = 0; i < static_cast<int>(info_.joints.size()); i++)
        {
            ...
            RCLCPP_INFO_STREAM(logger_, "Drive info: " << info_.joints[i].name << " Build date: " << info.buildDate << " tag: " << info.gitTag);
        }
    ...
    hardware_interface::CallbackReturn DynaArmHardwareInterface::on_deactivate_derived(const rclcpp_lifecycle::State & /*previous_state*/)
    {
